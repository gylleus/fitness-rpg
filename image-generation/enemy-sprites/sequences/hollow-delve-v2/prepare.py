...
Run with uv run sprite-python image-generation/enemy-sprites/sequences/hollow-delve-v2/prepare.py.
"""
...
def main():
    ...
    for enemy in roster["enemies"]:
        ...
        recipe = {"schema_version": 1, "asset_id": enemy["id"], "name": enemy["name"],
                  ...
                  "layout_note": "Alpha-connected extraction and fixed grid-column x anchor. Reviewed support contacts keep boots grounded when weapons extend below them. One scale and union crop across all 24 poses; no per-frame resizing.",
                  "actions": {}}
        ...
        for row, action in enumerate(ACTIONS):
            ...
            for i, (label, box) in enumerate(subjects[row*6:row*6+6]):
                ...
                contact_y = contacts[enemy["id"]][action][i]
                left = round(320 + x0 - anchor_x - i * column_width)
                top = 560 + y0 - contact_y
            ...
            audit["actions"][action] = {"source_boxes": boxes, "ground_contacts_y": contacts[enemy["id"]][action], "frame_count": 6}
