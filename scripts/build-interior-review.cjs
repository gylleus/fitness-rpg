...
const output = path.resolve(process.argv[3] || path.join(folder, 'review.html'));
...
async function main() {
  ...
  const groundEntry = Object.entries(sources).find(([, spec]) => Number.isFinite(spec.surface_y));
  if (!groundEntry) throw Error('Ground geometry is missing');
  const [groundKey, ground] = groundEntry;
  const textures = {};
  for (const [key, spec] of Object.entries(sources)) {
    ...
    textures[key === groundKey ? 'ground' : key] = { url: dataURL(file), size: spec.size };
  }
  ...
  const data = { title: props.biome_id.split('_').map(s => s[0].toUpperCase()+s.slice(1)).join(' '),
    assets: { scene, ground: { canvas: ground.canvas, surface_y: ground.surface_y }, props },
  ...
  console.log(`Standalone review: ${output}`);
}
