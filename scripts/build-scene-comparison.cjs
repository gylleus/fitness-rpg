...
if (!before || !after || !output) throw Error('Expected before directory, after directory and output HTML');
...
const cards = scenes.map(({ title, image }) => `<section><h2>${escape(title)}</h2><div class="pair"><figure><figcaption>Before</figcaption><img alt="${escape(title)} before" src="${embedded(before, image)}"></figure><figure><figcaption>Actor pixel scale</figcaption><img alt="${escape(title)} updated" src="${embedded(after, image)}"></figure></div></section>`).join('\n');
fs.mkdirSync(path.dirname(output), { recursive: true });
fs.writeFileSync(output, `<!doctype html><html lang="en"><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Biome pixel scale comparison</title>
...
<h1>Scenery at the characters’ pixel scale</h1><p>All six biomes use the same 1.5 pixels per game unit for scenery. Props are sized from their in-game height, with grouped color palettes. These are actual game-renderer captures at matching positions; player and enemy sprites are unchanged.</p><label><input type="checkbox" onchange="document.body.classList.toggle('large',this.checked)"> Larger stacked views</label>${cards}</html>`);
