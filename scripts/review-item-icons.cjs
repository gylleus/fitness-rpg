...
for (const category of new Set(items.map(item => item.category))) {
  ...
  const args = ['-font', process.env.FRPG_REVIEW_FONT || (process.platform === 'darwin' ? '/System/Library/Fonts/SFNS.ttf' : 'DejaVu-Sans'), '-pointsize', '11', '-fill', '#e8e1cc', '-background', '#18241f'];
...
const data = JSON.stringify(items.map(item => ({ ...item, image: 'data:image/png;base64,' + fs.readFileSync(path.join(root, `assets/items/${item.definitionId}.png`)).toString('base64') }))).replace(/</g, '\\u003c');
