...
export function LootRewards({ loot, secured = true }: { loot: readonly LootDrop[]; secured?: boolean }) {
  ...
  return <View style={{ gap: 10 }}>
    {displayed.map((drop, i) => <Pressable key={i} accessibilityRole="button" accessibilityLabel={`Preview loot ${drop.item.name}`}
      ...
      <View style={ui.flex}><Text style={ui.label}>{drop.boss ? 'Boss reward' : 'Loot'} · {secured ? 'In your bag' : 'Pending victory'}</Text>
        <Text style={[ui.body, { color: rarityColor[drop.item.rarity], fontWeight: '700' }]}>{drop.item.name}</Text>
        <Text style={ui.small}>Item level {drop.item.itemLevel ?? 1} · {itemStatsLabel(drop.item)}</Text>
      </View>
    </Pressable>)}
    {selected && <ItemDetails item={selected.item} onClose={() => setSelected(null)}>
      <Text style={[ui.body, { color: colors.green }]}>{secured ? 'Secured in your bag. Equip it from Inventory at camp.' : 'Pending loot. Defeat the final boss to secure this item.'}</Text>
    </ItemDetails>}
  </View>;
}
